...
-- Tabla de secciones (INVESTIGACIONES DESTACADAS, ARTÍCULOS RECIENTES,
-- PUBLICACIÓN DESTACADA, o las que quieras agregar).
create table if not exists public.sections (
  id uuid primary key default gen_random_uuid(),
  title text not null,                       -- Ej: "INVESTIGACIONES DESTACADAS"
  icon text not null default 'search',        -- search | book-open | pen-tool
  type text not null default 'carousel',      -- carousel | book
  link text not null default '/publicaciones',-- Enlace del botón "Ver todas"
  "order" integer not null default 0,         -- Posición en la página
  items jsonb not null default '[]'::jsonb,   -- Lista de ítems [{id,title,meta,image,link,description}]
...
-- Datos iniciales (puedes borrarlos y crear los tuyos desde el editor).
insert into public.sections (title, icon, type, link, "order", items)
values
(
  'INVESTIGACIONES DESTACADAS', 'search', 'carousel', '/publicaciones', 1,
  '[
...
(
  'ARTÍCULOS RECIENTES', 'book-open', 'carousel', '/publicaciones', 2,
  '[
...
(
  'PUBLICACIÓN DESTACADA', 'pen-tool', 'book', '/publicaciones', 3,
  '[
    {"id":"seed-pub-1","title":"Comunica, Lidera, Impacta","meta":"J. L. ZELADA","category":"Libro","author":"José Luis Zelada","date":"2022-06-01T00:00:00.000Z","featured":true,"tags":["Comunicación","Liderazgo","Influencia"],"link":"mailto:contacto@joseluiszelada.pe","description":"Estrategias prácticas para desarrollar liderazgo, comunicación efectiva e influencia positiva.","content":"Comunica, Lidera, Impacta es un libro pensado para profesionales y líderes que desean transformar su manera de comunicar y liderar. A través de estrategias prácticas, el lector aprende a desarrollar su presencia e influencia.\n\nEl libro se organiza en tres grandes pilares. Comunicar: cómo transmitir ideas con claridad y generar conexión. Liderar: cómo inspirar confianza y compromiso en los equipos. Impactar: cómo dejar una huella positiva y sostenible en la organización y la sociedad.\n\nCada capítulo combina teoría aplicada con ejercicios y casos reales, permitiendo al lector poner en práctica lo aprendido de inmediato.\n\nUna obra ideal para quienes buscan resultados concretos en su desarrollo personal y profesional."}
  ]'::jsonb
)
